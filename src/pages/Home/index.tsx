      ...
      {query.length > 0 ? (
        <div className="w-full grid grid-cols-1 md:grid-cols-2 p-10 gap-10">
          {filteredProducts.length > 0 ? (
            ...
            <h4 className="w-full md:col-span-2 text-3xl text-center">No products found</h4>
          )}
        </div>
      ) : (
        <InfiniteScroll
          className="w-full grid grid-cols-1 md:grid-cols-2 p-10 gap-10"
          dataLength={productsToShow.length}
          next={getNextPage}
          hasMore={hasMore}
          loader={<h4 className="w-full md:col-span-2 text-3xl text-center">Loading...</h4>}
          endMessage={<h4 className="w-full md:col-span-2 text-3xl text-center">No more products to show</h4>}
        >
